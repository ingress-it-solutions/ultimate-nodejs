...
module.exports = function(pg, ultimate, version = null) {
    patchClient(pg.Client, 'pg.Client', ultimate)
    ...
    if (getter) {
        ...
        pg.__defineGetter__('native', function() {
            ...
            if (native && native.Client) {
                patchClient(native.Client, 'pg.native.Client', ultimate)
            }
            return native
        })
    }
...
function patchClient(Client, klass, ultimate) {
    ...
    function wrapQuery(orig, name) {
        return function wrappedFunction(sql) {
            if (ultimate.isRecording()) {
                ...
                let segment = ultimate.startSegment('pgsql')
